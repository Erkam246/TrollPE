...
namespace Erkam246\TrollPE\Commands;
...
class TrollCommand extends PluginCommand {
    ...
    public function __construct(){
        parent::__construct("troll", TrollPE::getMain());
        $this->setPermission("trollpe.command");
        if(TrollPE::getMain()->getServer()->getName() === "Altay"){
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args){
        ...
        switch(strtolower($args[0])){
            case "chat":
                $pname = $args[1];
                if(!empty($pname)){
                    $player = TrollPE::getMain()->getServer()->getPlayer($pname);
                    if($player !== null){
                        if(isset($args[2])){
                            unset($args[0], $args[1]);
                            $msg = implode(" ", $args);
                ...
                if(!empty($pname)){
                    ...
                    if($player !== null){
                        ...
                        if(isset($args[2]) && is_int($multipler)){
                            TrollPE::getMain()->rocket($player, $multipler);
                        }else{
                            TrollPE::getMain()->rocket($player);
                        }
                ...
                if(!empty($pname)){
                    ...
                    if($player !== null){
                        ...
                        if(isset($args[2]) && is_int($radius)){
                            TrollPE::getMain()->blowup($player->getPosition(), $radius);
                        }else{
                            TrollPE::getMain()->blowup($player->getPosition());
                        }
            ...
            case "su":
                ...
                if(!empty($pname)){
                    ...
                    if($player !== null){
                        unset($args[0], $args[1]);
                        $cmd = implode(" ", $args);
                        TrollPE::getMain()->getServer()->dispatchCommand($player, $cmd, true);
